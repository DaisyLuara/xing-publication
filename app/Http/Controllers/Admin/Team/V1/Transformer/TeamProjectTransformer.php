...
class TeamProjectTransformer extends TransformerAbstract
{
    ...
    public function transform(TeamProject $teamProject)
    {
        return [
            ...
            'member' => [
                'interaction' =>array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'interaction';
                }),
                'originality' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'originality';
                }),
                'h5' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'h5';
                }),
                'animation' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'animation';
                }),
                'plan' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'plan';
                }),
                'tester' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'tester';
                }),
                'operation' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'operation';
                }),
                'others' => array_filter($teamProject->member->toArray(), function ($arr) {
                    return $arr['pivot']['type'] == 'others';
                }),
            ]
        ];
    }

    public function includeMember(){
