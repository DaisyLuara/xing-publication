...
class MarketingExport extends AbstractExport
{
    ...
    public function collection()
    {
        $faceCount = DB::connection('ar')->table('xs_face_count_log')
            ...
            ->selectRaw('ar_product_list.name as name,count(xs_face_count_log.oid) as pushNum ,sum(looknum) as lookNum ,sum(playernum7) as playerNum7,sum(playernum20) as playerNum20 ,sum(playernum) as playerNum,sum(lovenum) as loveNum,sum(outnum) as outNum,sum(scannum) as scanNum')
            ->get();
        $data = collect();
