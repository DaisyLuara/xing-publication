...
class OldMarketingExport extends AbstractExport
{
    ...
    public function collection()
    {
        $faceCount = DB::connection('ar')->table('xs_face_count_log')
            ...
            ->selectRaw('ar_product_list.name as name,count(xs_face_count_log.oid) as pushNum ,sum(playtimes7) as playtimes7,sum(playtimes15) as playtimes15,sum(playtimes21) as playtimes21,sum(looknum) as lookNum ,sum(playernum7) as playerNum7,sum(playernum20) as playerNum20 ,sum(playernum) as playerNum,sum(outnum) as outNum,sum(scannum) as scanNum,sum(omo_outnum) as omo_outnum,sum(omo_scannum) as omo_scannum,sum(phonenum) as phoneNum')
            ->get();
        $data = collect();
        $header1 = ['节目名称', '7″CPF', '', '15″CPF', '', '21″CPF', '', 'CPF', '', 'oCPF', '', 'CPR', '', '铁杆玩家', '', '生成数', '扫码数', 'CPA(去重)', 'CPA(不去重)', '扫码率', 'CPL', '', '1', '2', '5', '4', '10', '20', '合计'];
        ...
        $faceCount->each(function ($item) use (&$data) {
            ...
            $aa = [
                ...
                'rate' => (round(($item['playerNum'] == 0) ? 0 : $item['scanNum'] / $item['playerNum'], 2) * 100) . '%',
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                $cellArray = [
                    ...
                    'S1:S3', 'T1:T3', 'U1:V1', 'U2:U3', 'V2:V3', 'W2:W3',
                    'X2:X3', 'Y2:Y3', 'Z2:Z3', 'AA2:AA3', 'AB2:AB3', 'AC1:AC3'
                ];
                $event->sheet->getDelegate()->setMergeCells($cellArray);

                $event->sheet->getDelegate()
                    ->getStyle('A1:AC' . $this->data->count())
                    ->getAlignment()
