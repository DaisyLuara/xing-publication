...
class MarketingTopExport extends AbstractExport
{
    ...
    public function collection()
    {
        $faceCount1 = DB::connection('ar')->table('xs_face_count_log as fcl')
            ...
            ->selectRaw("apl.name as name,count(*) as days,sum(looknum) as looknum ,sum(playernum7)as playernum7,sum(playernum20) as playernum20 ,sum(playernum) as playernum,sum(lovenum) as lovenum,sum(outnum) as outnum,sum(scannum) as scannum");

        $faceCount2 = DB::connection('ar')->table(DB::raw("({$faceCount1->toSql()}) a,(select @gn := 0)  b"))
            ->selectRaw("  @gn := case when @name = name then @gn + 1 else 1 end gn,@name := name name,days,looknum,playernum7,playernum20,playernum,lovenum,outnum,scannum");

        $faceCount = DB::connection('ar')->table(DB::raw("({$faceCount2->toSql()}) c"))
            ->selectRaw("name,sum(days) as pushNum,sum(looknum) as lookNum,sum(playernum7) as playerNum7,sum(playernum20) as playerNum20,sum(playernum) as playerNum,sum(lovenum) as loveNum,sum(outnum) as outNum,sum(scannum) as scanNum")
            ->whereRaw("gn<=100")
            ->groupBy('name')
            ->get();
        $data = collect();
        ...
        $faceCount->each(function ($item) use (&$data) {
            ...
            $item = [
                ...
                'player' => $item['playerNum'] ? $item['playerNum'] : 0,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                $cellArray = [
                    'A1:A3', 'B1:C1', 'D1:E1', 'F1:G1', 'H1:I1',
                    'B2:B3', 'C2:C3', 'D2:D3', 'E2:E3', 'F2:F3',
                    'G2:G3', 'H2:H3', 'I2:I3', 'J1:J3', 'K1:K3',
                    'L1:L3', 'M1:N1', 'M2:M3', 'N2:N3', 'O2:O3',
                    'P2:P3', 'Q2:Q3', 'R2:R3', 'S2:S3', 'T2:T3',
                    'U1:U3',
                ];
