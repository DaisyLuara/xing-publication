...
class DemandModifyNotificationJob implements ShouldQueue
{
    ...
    public function handle()
    {
        $notification_params = [
            ...
            'type' => 'demand_modify',
            'reply_content' => "需求修改【" . $this->demandModify->getTitle() . '】有新的变更，请及时查看'
        ];
        ...
        if ($this->type == 'create' || $this->type == 'update') {
            ...
            if ($demand_application->getReceiverId()) {
                ...
                $demand_application->receiver->notify(new DemandNotification(
                    array_merge($notification_params,
                        ['reply_content' => "新的需求修改" . $type_text
                            . " -- 针对需求申请【" . $demand_application->getTitle() . "】有新的需求修改【" . $this->demandModify->getTitle() . "】" . $type_text . ",请及时处理"]
                    )));
            }
        } else if ($this->type == 'un_review') {
            //未反馈时 通知 审核人
            if (!$this->demandModify->getHasFeedback() && $reviewers) {
                $notification_params['reply_content'] = "需求修改超时未反馈 -- 针对需求申请【" . $demand_application->getTitle() . '】的需求修改【' . $this->demandModify->getTitle() . '】接单人超时未反馈，请查看处理';
            ...
            if ($this->demandModify->getStatus() == DemandModify::STATUS_PASS || $this->demandModify->getStatus() == DemandModify::STATUS_REJECT) {
                ...
                if ($this->demandModify->getStatus() == DemandModify::STATUS_PASS) {
                    $notification_params['reply_content'] = "需求修改已通过审核 -- 针对需求申请【" . $demand_application->getTitle() . "】的需求修改【" . $this->demandModify->getTitle() . "】由" . $this->demandModify->getReviewerName()
                        . "审核通过";
                } else if ($this->demandModify->getStatus() == DemandModify::STATUS_REJECT) {
                    $notification_params['reply_content'] = "需求修改审核不通过 -- 针对需求申请【" . $demand_application->getTitle() . "】的需求修改【" . $this->demandModify->getTitle() . "】由" . $this->demandModify->getReviewerName()
                        . "审核不通过；\n 不通过原因--" . $this->demandModify->getRejectRemark();
                }
