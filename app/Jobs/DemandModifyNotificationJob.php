...
class DemandModifyNotificationJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $str = ''
            ...
            . "  \n项目标的所属人：" . ($demand_application->owner_user ? $demand_application->owner_user->name : ' -- ')
        ...
        if ($this->type === 'create' || $this->type === 'update') {
            ...
            //未反馈时 通知 审核人
            if ($reviewers && !$this->demandModify->getHasFeedback()) {
            ...
            if ($this->demandModify->getHasFeedback()) {
                ...
                $demand_application->owner_user->notify(new BaseNotification($notification_params));
            ...
            if (in_array($this->demandModify->getStatus(), [DemandModify::STATUS_PASS, DemandModify::STATUS_REJECT], true)) {
