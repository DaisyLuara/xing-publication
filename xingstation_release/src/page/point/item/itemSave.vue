<template>
  <div class="item-wrap-template">
    ...
    <div class="pane"  :element-loading-text="setting.loadingText" v-loading="setting.loading">
      ...
      <el-form
        ref="pointForm"
        :model="pointForm" label-width="150px">
        <el-form-item label="场景" prop="scene" :rules="[{ required: true, message: '请输入节目名称', trigger: 'submit',type: 'number'}]">
          <el-select v-model="pointForm.scene" filterable placeholder="请搜索" >
            <el-option
        ...
        <el-form-item label="区域" prop="area"  :rules="[{ required: true, message: '请输入区域', trigger: 'submit' ,type: 'number'}]">
          <el-select v-model="pointForm.area" placeholder="请选择" filterable @change="areaChangeHandle">
            <el-option
        ...
        <el-form-item label="商场" prop="market" :rules="[{ required: true, message: '请输入商场', trigger: 'submit' ,type: 'number'}]">
          <el-select v-model="pointForm.market"  placeholder="请搜索" filterable :loading="searchLoading" remote :remote-method="getMarket" @change="marketChangeHandle">
            <el-option
        ...
        <el-form-item label="状态" prop="status"  :rules="[{ required: true, message: '请选择状态', trigger: 'submit' ,type: 'number'}]">
          <el-select v-model="pointForm.status" placeholder="请选择">
            <el-option
        ...
        <el-form-item
          label='图标'>
            <div class="up-area">
            <img v-if="thumbnail_url !== ''" :src="mediaBase + stepTwo.thumbnail_url" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon" @click="handleOpenPane()"></i>
            <i v-if="thumbnail_url !== ''" class="el-icon-circle-cross delete-icon-image" @click="handleImageDelete()"></i>
          </div>
        </el-form-item>
        ...
        <el-form-item label="类型" prop="type"  :rules="[{ required: true, message: '请选择类型', trigger: 'submit' ,type: 'number'}]">
          <el-select v-model="pointForm.type" placeholder="请选择">
            <el-option
  ...
  methods: {
    ...
    submit(formName) {
      this.$refs[formName].validate((valid) => {
        if(valid){
        this.setting.loading = true
          let edate = (new Date(this.pointForm.edate).getTime() + ((((23*60+59)*60)+59)*1000)) / 1000
          console.log(edate)
          let args = {
