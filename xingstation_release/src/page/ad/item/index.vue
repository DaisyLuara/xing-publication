                ...
                <el-table :data="scope.row.ad_plan.advertisements.data" style="width: 100%">
                    ...
                    <el-table-column :show-overflow-tooltip="true" label="素材显示格式" min-width="130">
                      <template slot-scope="ad_scope">
                        <span v-if="ad_scope.row.pivot">
                          ...
                          <br>
                          位置：{{ oriOptions[ad_scope.row.pivot.ori] }}
                          <br>
                  ...
                  >
                    <template slot-scope="ad_scope">
                      ...
                      <span
                        v-if="ad_scope.row.pivot"
                      >{{ (ad_scope.row.pivot.shm).toString().substring(ad_scope.row.pivot.shm.toString().length-2) }}</span>
                      至
                      <span
                        v-if="ad_scope.row.pivot"
                      >{{ (ad_scope.row.pivot.ehm).toString().substring(ad_scope.row.pivot.ehm.toString().length-2) }}</span>
                      分
                    </template>
                  </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getAreaList() {
      try {
        let res = await getSearchAera(this);
