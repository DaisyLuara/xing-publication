...
export default {
  ...
  methods: {
    ...
    async getAdPlanTimeDetail() {
      ...
      try {
        let res = await getAdPlanTimeDetail(this, args, this.planTimeId);
        this.adPlan = res.ad_plan;

        let adPlanTimeForm = res;
        ...
        if (this.adPlan.tmode === "hours") {
          this.time_format = "mm";
          adPlanTimeForm.shm = adPlanTimeForm.shm
            .toString()
            .substring(adPlanTimeForm.shm.toString().length - 2);
          adPlanTimeForm.ehm = adPlanTimeForm.ehm
            .toString()
            .substring(adPlanTimeForm.ehm.toString().length - 2);
        } else {
    ...
    async getAdPlanDetail() {
      //获取AdPlan 详情
      try {
        let res = await getAdPlanDetail(this, {}, this.planId);
        this.adPlan = res;
        if (this.adPlan.tmode === "hours") {
          this.time_format = "mm";
    ...
    async getSearchAdList() {
      let args = {
        atid: this.adPlan.atid
      };
      try {
        let res = await getSearchAdvertisement(this, args);
        this.searchAdList = res.data;
      } catch (e) {
        this.setting.loading = false;
      }
    },
    submit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.adPlan.tmode === "hours") {
            this.adPlanTimeForm.shm =
              "00:" +
              this.adPlanTimeForm.shm.substring(
                this.adPlanTimeForm.shm.length - 2
              );
            this.adPlanTimeForm.ehm =
              "00:" +
              this.adPlanTimeForm.ehm.substring(
                this.adPlanTimeForm.ehm.length - 2
              );
          }
          let { ...args } = this.adPlanTimeForm;
          args.atiid = this.adPlan.id;
          args.type = this.adPlan.type;
