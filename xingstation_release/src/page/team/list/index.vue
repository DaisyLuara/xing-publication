          ...
          <div 
            ...
            <el-form 
              ...
              <el-button
                ...
                size="small"
                class="button"
                @click="say(item.id)" >{{ item.attributes.name === 'ACTIVIEW' ? '团队成员' : item.attributes.name }}
              </el-button>
            </el-form>
          </div>
          <div 
            ...
            <div 
              ...
              <h1 
                ...
                {{ gropName }}
                <span 
                  style="font-size:14px;color: #999;">(共<b>{{ updateDate.length }}</b>人)</span>
              </h1>
            </div>
            <el-table 
              :data="updateDate"  
              ...
              <el-table-column
                ...
                <template 
                  slot-scope="scope">
                  <img 
                    :src="scope.row.attributes.gavatar" 
                    alt=""
                    class="icon-item"> 
                </template>
              </el-table-column>
              <el-table-column
                ...
                <template 
                  ...
                  <a 
                    class="member-name" 
                    href="javascript:;">{{ scope.row.attributes.nickname }}</a>
                  <el-tag 
                    v-for="item in scope.row.relationships.groups.data " 
                    :key="item.id" 
                    size="mini"
                    type="info" 
                    class="group-tag" >
                    {{ item.id | groupFilters(groupData) }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column
    ...
    say(id) {
      ...
      if (this.updateDate.length <= 0) {
        ...
        for (var i = 0; i < this.tableData.length; i++) {
          for (
            var j = 0;
            j < this.tableData[i].relationships.groups.data.length;
            j++
          ) {
